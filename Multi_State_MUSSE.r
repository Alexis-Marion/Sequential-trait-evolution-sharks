...
aicc_vec<-c(aicc(fit0$lnLik, length(argnames(lik0))), aicc(fit_sp1$lnLik, length(argnames(lik_sp1))), aicc(fit_sp2$lnLik, length(argnames(lik_sp2))))

habitat_binary_model_comp<-as.data.frame(cbind(c(fit0$lnLik, fit_sp1$lnLik, fit_sp2$lnLik), c(length(argnames(lik0)), length(argnames(lik_sp1)), length(argnames(lik_sp2))), aicc_vec, round(akaike.weights(aicc_vec)$deltaAIC, 3), round(akaike.weights(aicc_vec)$weights, 3)))

colnames(habitat_binary_model_comp)<-c("log likelihood", "number of parameters", "AICc", "Delta AIC", "AIC weights")
rownames(habitat_binary_model_comp)<-c("Null model", "Speciation 1", "Speciation 2")
